...
func TestStreamableStateless(t *testing.T) {
	...
	t.Run("stateless_server", func(t *testing.T) {
		...
		// Verify that session ID header is returned in stateless mode
		sessionID := resp.Header.Get(sessionIDHeader)
		if sessionID == "" {
			t.Error("Expected session ID header in stateless mode, got empty string")
		}
	...
	t.Run("delete_rejected_in_stateless", func(t *testing.T) {
		...
		if resp.StatusCode != http.StatusBadRequest {
			t.Errorf("Expected 400 Bad Request for DELETE in stateless mode, got: %d", resp.StatusCode)
		}
	})
}
